...
void mul_bonf(vector <double> pValue_ttest, vector <int> &Bonf_reject, INPUT input, map <int, int> ID, int dec)
{
    ...
    {
        if (pValue_ttest[l] <= input.alpha / (input.col + 0.0)) //reject if p[i]<= alpha/n
        {
...
void confusion(OUTPUT & output, vector <bool> result, vector <bool> truth, int regime, int experiment, int position, int dec)
{
    ...
    output.n_control[position] = regime;
    output.experiment[position] = experiment+1; //experiment starts from 0, so +1
    ...
    if (dec == 1) //if 1, show result.
    {
        ...
        cout << "n_control " << output.n_control[position] << " experiment " << output.experiment[position]
